...
export default {
  Query: {
    async currentUser(_, __, context) {
      ...
      logger.log({
        level: 'info',
        message: `got currentUser request for _id ${reqUser && reqUser._id}`,
      });
      const user = reqUser && Meteor.users.findOne(reqUser._id);
      if (user) {
        logger.log({
          level: 'info',
          message: `got currentUser with _id ${reqUser && reqUser._id}`,
        });
        return user;
      } else {
        logger.log({
          level: 'info',
          message: 'currentUser not found, returning {}',
        });
        return {};
      }
    },
    async users(
      ...
      logger.log({
        level: 'info',
        message: `got users request for _id ${reqUser && reqUser._id}`,
      });
      const user =
        reqUser &&
        first(
          Meteor.users
            .find({ _id: reqUser._id }, { fields: { admin: 1, username: 1 } })
            .fetch()
        );
      if (!user.admin) {
        logger.log({
          level: 'warn',
          message: `users requester with _id ${user._id} is no admin`,
        });
        return { usersCount: 0, usersList: [] };
      }
      logger.log({
        level: 'info',
        message: `returning users for _id ${user._id}`,
      });
      ...
      if (q.length === 0) {
        ...
        foundUsers = Meteor.users
          .find(
            {},
            {
              fields: { emails: 1, admin: 1, username: 1, createdAt: 1 },
              skip,
              limit: perPage,
              sort: opt,
            }
  ...
  Mutation: {
    async createUser(_, args, context) {
      ...
      logger.log({
        level: 'info',
        message: `got createUser request from _id ${user && user._id}`,
      });
      if (!user.admin) {
        logger.log({
          level: 'warn',
          message: `create user requester with _id ${user._id} is no admin`,
        });
      ...
      if (admin) {
        logger.log({
          level: 'info',
          message: `setting admin flag on user with _id ${userId}`,
        });
      ...
      logger.log({
        level: 'info',
        message: `created user ${JSON.stringify(newUser)}`,
      });
    ...
    async updateUser(_, args, context) {
      ...
      logger.log({
        level: 'info',
        message: `got updateUser request from _id ${user && user._id}`,
      });
      if (!user.admin) {
        logger.log({
          level: 'warn',
          message: `update user requester with ${user._id} is no admin`,
        });
        throw new Error('not authorized');
      }
      logger.log({
        level: 'info',
        message: `updating user with _id ${userId}, setting admin to ${admin}`,
      });
      Meteor.users.update({ _id: userId }, { $set: { username, admin } });
      if (password && password.length >= 8) {
        logger.log({
          level: 'info',
          message: `setting new password for user with _id ${userId}`,
        });
      ...
      if (admin !== adminBefore) {
        logger.log({
          level: 'info',
          message: `forcing user with _id ${userId} to logout since his admin status changed`,
        });
        Meteor.users.update(
          { _id: userId },
    ...
    async deleteUser(_, args, context) {
      ...
      logger.log({
        level: 'info',
        message: `got deleteUser request from _id ${reqUser && reqUser._id}`,
      });
      const user = reqUser && Meteor.users.findOne(reqUser._id);
      if (!user.admin) {
        logger.log({
          level: 'warn',
          message: `delete user requester with _id ${user._id} is no admin`,
        });
        throw new Error('not authorized');
      }
      ...
      if (userToDelete) {
        logger.log({
          level: 'info',
          message: `deleting user with _id ${userId}`,
        });
        try {
          Meteor.users.update(
            { _id: userId },
          ...
          logger.log({
            level: 'info',
            message: `deleted user with _id ${userId}`,
          });
          return true;
        } catch (err) {
          logger.log({
            level: 'error',
            message: `deleting user with _id ${userId} failed: ${JSON.stringify(
              err
            )}`,
          });
          return false;
        ...
        logger.log({
          level: 'info',
          message: `could not delete user with _id ${userId}`,
        });
        return false;
      }
    },
